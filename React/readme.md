...
- React render the page every time whenever the state is changed.
...
- React.memo is used to protect the component when no changes occur and it is change due to changes occur in parent here React.memo hook is play a very important to optimize your code.
- AVOID TO USE react.memo because it is also a overhead, it come with some addtional code and this make code more complex so use when there are lot of changes happen in main file.
- if we use props then if value of argument will change then React.memo will re-render the compoennt again, if it is a hard coded value then it will not re-render the component.
